 ...
 * <li>{@link com.weizoom.apiserver.cluster.handler.codec.HttpTaskDecoder}</li>用于从<code>http</code>中解析出<code>Task</code>
 * <li>{@link ExecutionHandler}</li>
 * <li>{@link com.weizoom.apiserver.cluster.operation.ClusterOperationHandler}</li>用于从处理<code>Operation</code>请求
 * <li>{@link com.weizoom.apiserver.cluster.handler.ExecuteHandlerForUser}</li>用于处理任务
 * <li>{@link HttpResponseEncoder}</li>用于加码的
 * <li>{@link com.weizoom.apiserver.cluster.handler.codec.HttpTaskResultEncoder}</li>用于加码的
 * @author wuyadong
 *
 */
